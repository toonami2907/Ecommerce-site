...
export const Navbar = () => {
  ...
  const {userinfo, cartupdate} = useGlobalContext()
...
const signOut = () =>{
  ...
  const handleSearch = async() => { 
    ...
    try {
      const response = await axios.get(`http://localhost:8080/product/v1/search/${search}`)
      ...
      if (error.response && error.response.data) {
        ...
        } else if (message.includes("Failed to get product, please try again later")) {
        ...
        <div className='flex items-center justify-between gap-10'>
          ...
          <div className='flex gap-5 font-light'>
            ...
            <button
              ...
              <ShoppingBagIcon />
              <span className='absolute text-xs px-2 py-1 -top-1 rounded-full text-white bg-purple-800 -right-1'>{cartupdate === undefined ? 0 : cartupdate }</span>
            </button>
            {/* CART FUNCTIONALITY */}
                ...
                <li key={idx}>
                  {/* <Link to={nav.to} className='text-3xl LinkStyles'>
                    {nav.name}
                     ...
                     <Link   onClick={() => setOpen(!open)} to={`/product/${nav.name}`} className='lg:text-3xl text-2xl LinkStyles'>{nav.name}</Link>
                </li>
              ))}
            </ul>
            {userinfo && userinfo ? 
            <div className='flex flex-col items-center gap-5'>
              <h1 className='text-xl font-semibold '>Welcome, {userinfo.username}</h1>
              <button   onClick={signOut} className='md:px-7 md:py-2 px-4 py-2 bg-red-700 hover:bg-red-500 rounded-full text-white text-lg'>sign out</button>
            </div> :  <div className='flex gap-5 py-7'>
              <button   onClick={() => setOpen(!open)} className='md:px-7 md:py-2 px-4 py-2 bg-black hover:bg-gray-700 rounded-full text-white text-lg'><Link to="/register">Sign up</Link></button>
              <button   onClick={() => setOpen(!open)} className='md:px-7 md:py-2 px-4 py-2 border border-gray-600 hover:border-black rounded-full text-lg text-gray-800'><Link to="/Login">Sign in</Link></button>
            </div>}
           
          </div>
        </div>
        {/* SEARCH BOX FUNCTIONALITY */}
        <div className={`fixed inset-0  transition-transform transform ${searchbox ? 'translate-y-0' : 'translate-y-full'} bg-opacity-30 lg:flex hidden bg-black backdrop-blur-sm z-50`}>
