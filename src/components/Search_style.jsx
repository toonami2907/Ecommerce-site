    ...
    <div>
        ...
        {data && data.map((item, idx)=>(
            <li key={idx}  >
                ...
                <p className='p-style'>Stock: <span className='span-style'>{item.stock}</span></p>
                <p className='p-style'> Price: <span className='span-style'>${item.price}</span></p>
                <p className={`${item.size === 0 ? "hidden" : "flex gap-2 p-style"}`}>Size: {item.size.map((siz,i)=>(
                  <>
                  <span key={i} className='flex span-style'>{siz}</span>
                  </>
                ))}</p>
                </Link>
            </li>
